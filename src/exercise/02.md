...
useEffect is called after the DOM has been updated and also the browser has
finished painting.

### useLayoutEffect

useLayoutEffect is called after the DOM has been updated doesn't care if the
browser has finished painting. Used when we want to mutate the DOM or trigger a
synchronous re-render

NB! It stops the browser from paiting the DOM
