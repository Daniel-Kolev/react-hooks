...
Second argument to .then() catches only the errors from the function returning
the promise but not the resolved callback function

If you want to handle both functions then use .catch()

ErrorBoundary component can help us lift our error handling logic out of the
component by throwing errors and showing a nice looking UI to the user.
